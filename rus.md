...
![Background applied to CSS shape][9]
...
![Background applied to rectangular box model][10]
Фон применется к форме прямоугольной коробки элемента
...
Мы установим высоту равную высоте контейнера, которому для этого примера я установила высоту вьюпорта используя css `vh` блок. 
...
Сейчас все что мы должны сделать это объявить фигуру на плавающем элементе так чтобы текст обступал его с этой стороны.
...
Для примера, вместо использования функции polygon() для определения фигуры, мы зададим свойству `shape-outside` image URI, и браузер достанет фигуру из изображения и использует ее.

Изображение, которое будет определять область исключения для этого примера представлено ниже. 
