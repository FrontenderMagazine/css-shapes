...
Чтобы использовать CSS-формы на элементах, мы сначала должны установить систему координат, которая будет использоваться для рисования форм или фигур.

Система координат обязательна, так как фигуры, которые ты описываешь будут определены через набор точек (и радиусов, если ты будешь рисовать круги или элипсы, например) и эти точки имеют кооридинаты `x` and `y`, которые располагаются на координатной оси.

Свойства shape-* используют контейнер содержимого элемента и применяют к нему систему координат, поэтому для того, чтобы заставить их работать вам нужно указать фиксированную ширину и высоту для элемента, которые определяет границы, которые в свою очередь будут использоваться для установления системы координат для отрисовки фигур. **Если явно ширина и высота не указаны свойства фигур работать не будут.**
