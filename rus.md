...
На примере ниже видно изображение, которое отображает исключающую область. Вы можете заметить что это изображение повторяет фигуру созданную с помощью функции `polygon()` из точек примером выше. 
...
Когда мы используем изображения с альфа каналом для определения формы через свойство shape-outside, прозрачная область изображения будет обозначать область, где текст будет обрамлять форму, это область будет называться *плавающей областью* элемента. Черная часть определяет область исключения элемента. 
