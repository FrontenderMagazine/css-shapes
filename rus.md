...
Если свойство установлено в «функцию формы(shape function)», то форма вычисляется на основе выбора одного из значений функций - `rectangle`, `inset-rectangle`, `circle`, `ellipse`, `polygon`. Вы можете узнать намного больше об этих функциях в [статье][6] от команды Adobe Platform.

И наконец, если у свойста установлено URI изображения, браузер будет использовать изображение для извлечения и вычисления формы, основываясь на альфа-канале изображения. Форма вычисляет путь, который охватывает область, где непрозрачность указаного изображения больше, чем значение параметра `shape-image-threshold`. Если параметр `shape-image-threshold` не указан, то начальное значение будет установлено в `0.5`. Изображение должно быть CORS-same-origin, в противном случае оно не будет работать, и значение автоматически установится в `auto`.
...
Свойство `clip-path` используется для *отсечения* частей элемента, которые нам не понадобятся. Очевидно, что это означает, что мы не применяем цвет к форме, а только *обрезаем* элемент и оставляем нетронутой форму. При этом текст будет обтекать круг.
...
Скриншот демо #1. Нажмите на скриншот, чтобы увидеть работающее демо
...
Скриншот на котором изображены вершины, которые образуют прямоугольную форму
...
![Изображение с альфа-каналом для определения формы из демо#3][34]
Изображение с альфа-каналом для определения формы из демо#3
...
<figcaption>Иллюстрация набора точек для создания одного многоугольника</figcaption>
...
<figcaption>Подобный макет может быть воссоздан при помощи CSS-форм и регионов</figcaption>
